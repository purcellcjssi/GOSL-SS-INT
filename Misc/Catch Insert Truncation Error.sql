...
/*
        SELECT t.event_id
            , CASE WHEN LEN(t.emp_id) > 15 THEN t.emp_id ELSE '' END emp_id
            , CASE
                WHEN LEN(RTRIM(t.eff_date)) < 8 THEN @v_BAD_DATE_INDICATOR
                ELSE COALESCE(TRY_CONVERT(datetime, t.eff_date), @v_BAD_DATE_INDICATOR)
              END AS eff_date
            , CASE WHEN LEN(t.first_name) > 25 THEN t.first_name ELSE ''  END  AS first_name
            , CASE WHEN LEN(t.first_middle_name) > 25 THEN t.first_middle_name ELSE '' END AS first_middle_name
            , CASE WHEN LEN(t.last_name) > 30 THEN t.last_name ELSE '' END AS last_name
            , CASE WHEN LEN(UPPER(t.empl_id)) > 10 THEN t.empl_id ELSE '' END AS empl_id
            , CASE WHEN LEN(t.national_id_type_code) > 5 THEN t.national_id_type_code ELSE '' END AS national_id_type_code
            , CASE WHEN LEN(t.national_id) > 20 THEN t.national_id ELSE '' END AS national_id
            , COALESCE(TRY_CONVERT(int, t.organization_group_id), 0) AS organization_group_id
            , @v_EMPTY_SPACE AS organization_chart_name     -- t.organization_chart_name  -- wrong value
            , @v_EMPTY_SPACE AS organization_unit_name      -- t.organization_unit_name
            , CASE WHEN LEN(t.emp_status_classn_code) > 2 THEN t.emp_status_classn_code ELSE '' END AS emp_status_classn_code
            , LEFT(t.position_title, 50) AS position_title    -- trim value since HCM sends it over as char(60)
            , CASE WHEN LEN(UPPER(t.employment_type_code)) > 70 THEN t.employment_type_code ELSE '' END AS employment_type_code
            , COALESCE(TRY_CONVERT(money, t.annual_salary_amt), 0.00) AS annual_salary_amt
            , CASE
                WHEN LEN(RTRIM(t.begin_date)) < 8 THEN @v_BAD_DATE_INDICATOR
                ELSE COALESCE(TRY_CONVERT(datetime, t.begin_date), @v_BAD_DATE_INDICATOR)
            ...
            , CASE
                ...
                ELSE COALESCE(TRY_CONVERT(datetime, t.end_date), @v_BAD_DATE_INDICATOR)
                END AS end_date
            , CASE WHEN LEN(t.pay_status_code) > 1 THEN t.pay_status_code ELSE '' END AS pay_status_code
            , CASE WHEN LEN(UPPER(t.pay_group_id)) > 10 THEN t.pay_group_id ELSE '' END AS pay_group_id
            , CASE WHEN LEN(t.pay_element_ctrl_grp_id) > 10 THEN t.pay_element_ctrl_grp_id ELSE '' END AS pay_element_ctrl_grp_id
            , CASE WHEN LEN(t.time_reporting_meth_code) > 1 THEN t.time_reporting_meth_code ELSE '' END AS time_reporting_meth_code
            , CASE WHEN LEN(t.employment_info_chg_reason_cd) > 5 THEN t.employment_info_chg_reason_cd ELSE '' END AS employment_info_chg_reason_cd
            , CASE WHEN LEN(t.emp_location_code) > 10 THEN t.emp_location_code ELSE '' END AS emp_location_code
            , CASE WHEN LEN(t.emp_status_code) > 2 THEN t.emp_status_code ELSE '' END AS emp_status_code
            , CASE WHEN LEN(t.reason_code) > 2 THEN t.reason_code ELSE '' END AS reason_code
            , t.emp_expected_return_date
            , CASE
                WHEN LEN(RTRIM(t.pay_through_date)) < 8 THEN @v_BAD_DATE_INDICATOR
                ELSE COALESCE(TRY_CONVERT(datetime, t.pay_through_date), @v_BAD_DATE_INDICATOR)
            ...
            , CASE
                ...
                ELSE COALESCE(TRY_CONVERT(datetime, t.emp_death_date), @v_BAD_DATE_INDICATOR)
              END AS emp_death_date
            , CASE WHEN LEN(t.consider_for_rehire_ind) > 1 THEN t.consider_for_rehire_ind ELSE '' END AS consider_for_rehire_ind
            , CASE WHEN LEN(UPPER(t.pay_element_id)) > 10 THEN t.pay_element_id ELSE '' END AS pay_element_id
            , COALESCE(TRY_CONVERT(money, t.emp_calculation), 0.00) AS emp_calculation
            , CASE WHEN LEN(t.tax_flag) > 1 THEN t.tax_flag ELSE '' END AS tax_flag        -- CASE t.tax_flag WHEN '1' THEN 'Y' WHEN '0' THEN 'N' ELSE tax_flag END tax_flag
            , CASE WHEN LEN(t.nic_flag) > 1 THEN t.nic_flag ELSE '' END AS nic_flag        -- CASE t.nic_flag WHEN '1' THEN 'Y' WHEN '0' THEN 'N' ELSE nic_flag END nic_flag
            , COALESCE(TRY_CONVERT(money, t.tax_ceiling_amt), 0.00) AS tax_ceiling_amt
            , CASE WHEN LEN(t.labor_grp_code) > 50 THEN t.labor_grp_code ELSE '' END AS labor_grp_code
            , CASE WHEN LEN(t.file_source) > 50 THEN t.file_source ELSE '' END AS file_source
            , COALESCE(TRY_CONVERT(money, t.annual_hrs_per_fte), 0.00) AS annual_hrs_per_fte
            , COALESCE(TRY_CONVERT(money, t.annual_rate), 0.00) AS annual_rate
            , CASE
                WHEN LEN(RTRIM(t.birth_date)) < 8 THEN @v_BAD_DATE_INDICATOR
                ELSE COALESCE(TRY_CONVERT(datetime, t.birth_date), @v_BAD_DATE_INDICATOR)
              END AS birth_date
            , CASE WHEN LEN(t.gender) > 1 THEN t.gender ELSE '' END AS gender
            , CASE t.country_code WHEN 'LCA' THEN 'EC1' ELSE 'GN4' END addr_fmt_code    -- derive address format code based on country code
            , LEFT(t.country_code, 2) AS country_code
            , LEFT(t.addr_line_1, 35) AS addr_line_1
            , LEFT(t.addr_line_2, 35) AS addr_line_2
            , CASE WHEN LEN(CASE t.country_code WHEN 'LCA' THEN LTRIM(RTRIM(t.addr_line_3 + ' ' + t.addr_line_4)) ELSE t.addr_line_3 END) > 35 THEN CASE t.country_code WHEN 'LCA' THEN LTRIM(RTRIM(t.addr_line_3 + ' ' + t.addr_line_4)) ELSE t.addr_line_3 END ELSE '' END AS addr_line_3        -- combine line 3 and 4 if St Lucia
            , CASE WHEN LEN(CASE t.country_code WHEN 'LCA' THEN @v_EMPTY_SPACE ELSE t.addr_line_4 END) > 35 THEN CASE t.country_code WHEN 'LCA' THEN @v_EMPTY_SPACE ELSE t.addr_line_4 END ELSE '' END AS addr_line_4
            , LEFT(t.city_name, 35) AS city_name
            , LEFT(t.state_prov, 9) AS state_prov
            , LEFT(t.postal_code, 9) AS postal_code
            , CASE WHEN LEN(t.county_name) > 255 THEN t.county_name ELSE '' END county_name
            , CASE WHEN LEN(t.region_name) > 255 THEN t.region_name ELSE '' END region_name
            , DBShrpn.dbo.ufn_ret_job_or_pos_id(t.file_source, t.empl_id) AS job_or_pos_id
			        FROM DBShrpn.dbo.ghr_employee_events t
*/
