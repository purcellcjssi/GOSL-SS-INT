...
CREATE OR ALTER PROCEDURE dbo.usp_pay_check_deduction
AS
BEGIN
   ...
   SELECT CONVERT(CHAR,@end_of_period,112)          AS EffectiveDate
        , 'GOSL'                                    AS HROrganization      -- CJP 5/22/2025 changed value from 'GOG'
   ...
   WHERE d.pay_element_type_code = '2' 
     AND d.pay_element_descp <> 'Net Pay'
   ORDER BY e.check_nbr
   

   SELECT @max      =  COUNT(*) 
   FROM DBShrpn.dbo.ghr_deduction_temp
   

   SELECT @oldchk   =   CheckID 
   FROM DBShrpn.dbo.ghr_deduction_temp 
   WHERE   ID = 1
   ...
   BEGIN
      ...
      IF @oldchk <> @newchk
         BEGIN
            ...
            UPDATE DBShrpn.dbo.ghr_deduction_temp 
            SET Deduction = CONVERT(CHAR(1),@ncnt)   
            WHERE ID = @skey
