...
CREATE procedure [dbo].[usp_pay_slip_interface]
...
AND ep.payroll_run_type_id like '%MONTHLY%'
AND ep.check_date = (SELECT MAX(check_date) FROM [DBShrpy].[dbo].[emp_pmt] WHERE pmt_type_code = '01' AND payroll_run_type_id like '%MONTHLY%' ) 
...
FROM DBShrpy.dbo.emp_pmt ep
WHERE ep.seq_ctrl_yr = 2021 --@current_year
AND ep.pmt_type_code = '01'
AND ep.payroll_run_type_id like '%MONTHLY%'
AND ep.check_date = (Select MAX(check_date) from DBShrpy..emp_pmt t WHERE t.emp_id=ep.emp_id AND t.payroll_run_type_id = 'MONTHLY' AND t.pmt_type_code = '01'  )  
...
SELECT DISTINCT pay_pd_id FROM [DBShrpy].[dbo].[ghr_payment_temp] 
WHERE check_date = (SELECT MAX(check_date) FROM [DBShrpy].[dbo].[ghr_payment_temp])
...
where ep.seq_ctrl_yr = 2021 --@current_year
and ep.pmt_type_code = '01'
and ep.payroll_run_type_id like '%UNESMONTH%'
AND ep.check_date = (Select MAX(check_date) from DBShrpy..emp_pmt t WHERE t.emp_id=ep.emp_id AND t.payroll_run_type_id = 'UNESMONTH' AND t.pmt_type_code = '01'  ) 
...
AND ep.check_date = (Select MAX(check_date) from DBShrpy..emp_pmt t WHERE t.emp_id=ep.emp_id AND t.payroll_run_type_id = 'ALLOW' AND t.pmt_type_code = '01'  )
...
SELECT @max = COUNT(ID) FROM [DBShrpy].[dbo].[ghr_pay_period_temp]

DELETE DBShrpn.dbo.ghr_msg_tbl 

WHILE (@cnt <= @max)
BEGIN
	SELECT	@pay_pd_id	=	[pay_pd_id], @skey	=	[ID]
	  FROM [DBShrpy].[dbo].[ghr_pay_period_temp] t 
	 WHERE	t.ID = @cnt
	  

	EXEC DBShrpy.dbo.usp_payslip_ext @p_pay_period_id = @pay_pd_id, @p_check_nbr = '000000'    
