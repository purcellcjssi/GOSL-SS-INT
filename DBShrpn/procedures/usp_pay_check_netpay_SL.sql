...
CREATE OR ALTER PROCEDURE dbo.usp_pay_check_netpay
AS
BEGIN
   --
   -- Pay Check Net Pay
   ...
   IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'dbo.ghr_emp_pmt_sumn_temp') AND type in (N'U'))
      DROP TABLE dbo.ghr_emppmtsumn_temp

   CREATE TABLE DBShrpn.dbo.ghr_emppmt_sumn_temp 
   (
   ...
   FROM DBSbank.dbo.pmt_ref_ctrl
   WHERE bank_id       = 'ACCPAY'         -- CJP 5/22/2025 'PAYSLIP'
     AND bank_acct_nbr = '999999'         -- CJP 5/22/2025 '9999999'
     AND type_of_pmt   = 'HRPK'           -- CJP 5/22/2025 'CHK'
     AND pmt_meth_id   = 'CHECK'          -- CJP 5/22/2025 'PAYSL'
   ...
   SELECT @max_check_nbr =   MAX(CONVERT(INT,check_nbr))
   FROM DBShrpn.dbo.ghr_emp_pmt_sumn_temp
